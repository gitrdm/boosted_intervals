...
## Numeric types and integer semantics

Note: Boost.Interval supports both integer and real interval types at the C++
level. This package, however, assumes real (double) interval endpoints for all
operations because unit conversions and the `units` package are defined for
reals. Integer-specific behaviours (discrete wrapping, overflow semantics,
or other integer-only rules) are not modelled by the R wrappers. If you need
integer-typed interval semantics, use Boost directly or extend the C++ layer.
...
Boost's comparison helpers surface fine-grained reasoning about interval
relations. `possible()` checks whether a relation can hold, `certain()` tests
whether it must hold, and `verify()` combines both to deliver a tri-state
answer. The new `contract()` helper shrinks bounds symmetrically, while
`as_numeric_bounds()` emits plain numeric endpoints for interop with APIs that
expect doubles.

```{r}
a <- units_interval(set_units(c(0, 2), "m"), set_units(c(0.5, 4), "m"))
b <- units_interval(set_units(c(1, 1), "m"), set_units(c(2, 3), "m"))
possible(a, b, "<")
certain(a, b, "<")
verify(a, b, "<")

enclosure <- units_interval(set_units(0, "s"), set_units(10, "s"))
contract(enclosure, absolute = set_units(1, "s"))
as_numeric_bounds(enclosure, unit = "ms")
```
