...
Utilities such as `width()`, `midpoint()`, `radius()`, `mag()`, `mig()`, and
`distance()` return `units` vectors that can be fed directly into plotting
pipelines or summary tables, while `zero_in()` surfaces Boost's diagnostic for
