...
Utilities such as `width()`, `midpoint()`, `radius()`, `norm()`, `mag()`,
`mig()`, and `distance()` return `units` vectors that can be fed directly into
plotting pipelines or summary tables, while diagnostics like `zero_in()` and
`is_whole()` surface common Boost predicates.
